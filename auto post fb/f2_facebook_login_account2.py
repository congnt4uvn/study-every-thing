...
SESSION_DIR = Path("session_data_account2")  # Separate session directory for account 2
...
def restore_session(driver: webdriver.Chrome, group_url: str) -> bool:
    print("[LOGIN ACCOUNT 2] Navigating to group page...")
    driver.get(group_url)
    time.sleep(2)
    
    print("[LOGIN ACCOUNT 2] Restoring cookies...")
    ...
    if restored_cookies and isinstance(storage_data, dict):
        print("[LOGIN ACCOUNT 2] Restoring storage and refreshing once...")
        restore_storage(driver, storage_data)
        driver.refresh()
        time.sleep(2)
    elif restored_cookies:
        print("[LOGIN ACCOUNT 2] Refreshing page...")
        driver.refresh()
        time.sleep(2)
    
    logged_in = is_logged_in(driver)
    print(f"[LOGIN ACCOUNT 2] Login status: {'✓ Success' if logged_in else '✗ Failed'}")
    return logged_in


def wait_for_manual_login(driver: webdriver.Chrome, timeout_seconds: int = LOGIN_TIMEOUT_SECONDS) -> bool:
    driver.get(FACEBOOK_URL)
    print(f"[ACCOUNT 2] Please log in manually (timeout: {timeout_seconds}s)")
    try:
...
def login_and_save(driver: webdriver.Chrome, group_url: str) -> bool:
    ...
    if restore_session(driver, group_url):
        print("[ACCOUNT 2] Session restored")
        return True
    else:
        if not wait_for_manual_login(driver):
            print("[ACCOUNT 2] Login timeout")
            return False
        save_session(driver)
        print("[ACCOUNT 2] Session saved")
        return True


if __name__ == "__main__":
    # Test login functionality for Account 2
    driver = build_driver()
    try:
